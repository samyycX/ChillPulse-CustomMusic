...
{
    public static string gml_Object_obj_controller_Draw_64 = """
    variables()
    ...
    if (page_music_list == 1)
    {
        ...
        if (page_full_music == 1)
        {
            ...
            var _center_y1 = 155
            var cell_size = 123
            var _music_genre = [_text_all_music, _text_cyberpunk, _text_chinese_music, _text_custom, _text_classical, _text_ambient%MUSICGENRE%]
    ...
    if (timer_run == true)
    {
        ...
        if (pomodoro_minute <= 0)
        {
            ...
            var a = dialogue[current_scene][_nora]
            var b = current_sentence[current_scene][_nora]
            var c = a[(b + 1)]
                            ...
                            if ((i + diary_line_start * 3) == 0)
                            {
                                ...
                                if button_collision(324, 238, 397, 340)
                                {
                                    ...
                                    if mc_click
                                    {
                                        ...
                                        var new_diary = {title: _text_new_diary, cover: 0, content: []}
    ...
    if (page_phone == 1)
    {
        ...
        if (phone_page_type != "chat")
        {
            ...
            for (ii = 0; ii < array_length(current_sentence[current_scene]); ii++)
            {
                ...
                {
                    var a = dialogue[current_scene][ii]
                    var b = current_sentence[current_scene][ii]
                    if (a[(b + 1)][1] != "Me" && typeof(a[(b + 1)][1]) != "number")
        ...
        switch phone_page_type
        {
            case "main":
                ...
                for (i = 0; i < array_length(phone_contact_list[current_scene]); i++)
                {
                    if (current_sentence[current_scene][i] != -1)
                    {
                        ...
                        {
                            var a = dialogue[current_scene][i]
                            var b = current_sentence[current_scene][i]
                            if (a[(b + 1)][1] != "Me" && typeof(a[(b + 1)][1]) != "number")
            ...
            case "chat":
                ...
                color = c_black
                draw_set_valign(fa_middle)
                var a = current_sentence[current_scene]
                var b = current_contact[current_scene]
                var c = a[b]
                var d = dialogue[current_scene]
                var e = d[b]
                if (c >= array_length(e))
                {
                    c = array_length(e) - 1
                    sentence_show_start[current_scene][b] = max(0, (c - 5))
                    show_debug_message("got you")
                }
                if (button_collision(0, 0, 500, 720) && c >= 6)
                {
                    if mouse_wheel_down()
                        sentence_show_start[current_scene][b] = min((sentence_show_start[current_scene][b] + 1), (current_sentence[current_scene][b] - 5))
                    if mouse_wheel_up()
                        sentence_show_start[current_scene][b] = max((sentence_show_start[current_scene][b] - 1), 0)
                }
                if (c >= 5)
                {
                    draw_set_alpha(0.5)
                ...
                for (i = 0; i < min(6, (c + 1)); i++)
                {
                    var text_y = phone_y + 120 + i * 50
                    var text_to_draw = e[(i + sentence_show_start[current_scene][b])][0]
                    var textbox_length = string_width(text_to_draw) * text_scale
                    if (e[(i + sentence_show_start[current_scene][b])][0] != "")
                    {
                        if (e[(i + sentence_show_start[current_scene][b])][1] == "Me")
                        {
                            draw_set_halign(fa_right)
                ...
                {
                    var a = dialogue[current_scene]
                    var b = current_contact[current_scene]
                    var c = current_sentence[current_scene]
                    var e = c[b]
                    var f = a[b]
                    var next_text_to_draw = f[(e + 1)][0]
        ...
        switch page_scene_state
        {
            ...
            case "typename":
                ...
                if button_request_new((display_get_gui_width() / 2), (display_get_gui_height() / 2 + 200), _text_yes)
                {
                    ...
                    var _scene_to_save = {scene_name: current_scene_name_to_save,current_scene: current_scene,current_girl_cloth: current_girl_cloth[current_scene],current_head_wearing: current_head_wearing,current_pet: {pet: argument[4],category: argument[5]}}
